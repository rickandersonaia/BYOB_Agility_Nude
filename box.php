...
class byobagn_easy_header extends thesis_box {
        ...
        protected function options() {
                ...
                $options['column_content_2']['parent'] = array('layout' => array('columns_2', 'columns_321', 'columns_312', 'columns_413', 'columns_431', 'columns_3', 'columns_4121', 'columns_4211', 'columns_4112', 'columns_4'));
                $options['column_content_3']['parent'] = array('layout' => array('columns_3', 'columns_4121', 'columns_4211', 'columns_4112', 'columns_4'));
...
class byobagn_easy_responsive_columns extends thesis_box {
        ...
        protected function html_options() {
                ...
                $thesis_options = $thesis->api->html_options(array(
                    ...
                    'section' => 'section',
                    'nav' => 'nav'), 'div');
                $thesis_options['html']['label'] = __('HTML tag for column wrapper', 'byobbase');
                $thesis_options['class']['label'] = __('HTML <code>class</code> for column wrapper', 'byobbase');
                $thesis_options['id']['label'] = __('HTML <code>id</code> for column wrapper', 'byobbase');
                $layout_options['layout'] = array(
                    'type' => 'select',
                    'label' => __('Select the column configuration for this column wrapper', 'byobbase'),
                    'options' => array(
                        'columns_1' => __('Single column', 'byobbase'),
                        'columns_2' => __('Two columns, equally sized', 'byobbase'),
                        'columns_321' => __('Two columns, two thirds - one third', 'byobbase'),
                        'columns_312' => __('Two columns, one third - two thirds', 'byobbase'),
                        'columns_431' => __('Two columns, three quarters - one quarter', 'byobbase'),
                        'columns_413' => __('Two columns, one quarter - three quarters', 'byobbase'),
                        'columns_3' => __('Three columns, equally sized', 'byobbase'),
                        'columns_4211' => __('Three columns, one half - one quarter - one quarter', 'byobbase'),
                        'columns_4121' => __('Three columns, one quarter - one half - one quarter', 'byobbase'),
                        'columns_4112' => __('Three columns, one quarter - one quarter - one half', 'byobbase'),
                        'columns_4' => __('Four columns, equally sized', 'byobbase')
                    ),
                    'dependents' => array('columns_312', 'columns_413', 'columns_4121', 'columns_4112')
                );
                $layout_options['reverse'] = array(
                    'type' => 'checkbox',
                    'label' => __('Keep content column first on mobile', 'byobbase'),
                    'tooltip' => __('By checking this box the content column will display first on smaller mobile devices', 'byobbase'),
                    'options' => array(
                        'use' => __('Display content first', 'byobbase')
                    ),
                    'parent' => array(
                ...
                $options['use_area'] = array(
                    'type' => 'checkbox',
                    'label' => __('Add the "area wrapper" around these columns', 'byobbase'),
                    'tooltip' => __('By checking this box you will create a wrapping div with the class of "area wrapper" - which is the main full screen horizontal element', 'byobbase'),
                    'options' => array(
                        'use' => __('Wrap this with "area wrapper"', 'byobbase')
                    ),
                ...
                $options['area_id'] = array(
                    ...
                    'width' => 'medium',
                    'code' => 'true',
                    'label' => __('Add an id to the "area wrapper" - optional', 'byobbase'),
                    'tooltip' => __('This id will be added to the div with the class of "area wrapper" - which is the main full screen horizontal element', 'byobbase'),
                    'parent' => array(
                        'use_area' => 'use')
                );
                $options['area_class'] = array(
                    ...
                    'width' => 'medium',
                    'code' => 'true',
                    'label' => __('Add another class to the "area wrapper" - optional', 'byobbase'),
                    'tooltip' => __('This class will be added to the div with the class of "area wrapper" - which is the main full screen horizontal element', 'byobbase'),
                    'parent' => array(
                        'use_area' => 'use')
                );
                ...
                $options['overlay'] = array(
                    'type' => 'checkbox',
                    'label' => __('Add an Overlay element', 'byobbase'),
                    'tooltip' => __('By checking this box you will add an overlay div between the area and the page wrapper', 'byobbase'),
                    'options' => array(
                        'add' => __('Add an overlay element', 'byobbase')
                    ),
                ...
                $options['page_wrapper'] = array(
                    'type' => 'checkbox',
                    'label' => __('Remove Page Wrapper', 'byobbase'),
                    'tooltip' => __('Choose the configuration for the page wrapper.  If you remove the page wrapper the element will be full width', 'byobbase'),
                    'code' => true,
                    'options' => array(
                        'remove' => __('Remove the page wrapper', 'byobbase')
                ));

                $options['remove_padding'] = array(
                    'type' => 'checkbox',
                    'label' => __('Remove the typical padding from the columns', 'byobbase'),
                    'tooltip' => __('By checking this box you will add the class of "frameless" to the column wrapper', 'byobbase'),
                    'options' => array(
                        'remove' => __('Remove the typcial padding', 'byobbase')
                    )
                );

                $options['v_padding'] = array('type' => 'group',
                    'label' => __('Remove top & bottom padding from column wrapper', 'byobbase'),
                    'fields' => array(
                        'v_padding' => array(
                            'tooltip' => __('Theses options will remove default padding from the top and bottom of the column wrapper', 'byobbase'),
                            'type' => 'checkbox',
                            'options' => array(
                                'padding-top' => __('Remove top padding', 'byobbase'),
                                'padding-bottom' => __('Remove bottom padding', 'byobbase'),
                ))));
...
class byobagn_content_grid extends thesis_box {
        ...
        protected function options() {
                ...
                $options['layout'] = array(
                    ...
                    'dependents' => array('columns_2', 'columns_321', 'columns_312', 'columns_413', 'columns_431', 'columns_3', 'columns_4121', 'columns_4211', 'columns_4112', 'columns_4')
                );
                $count = 1;
                while ($count < 5) {
                        $options['column_content_' . $count] = array(
...
class byobagn_related_posts_thumbnails extends thesis_box {
        ...
        public function html($args = array()) {
                ...
                while ($this->query->have_posts()) {
                        ...
                        echo "$tab\t<$html$class$schema_att>\n";
                        $this->rotator(array_merge($args, array('depth' => $depth + 2, 'schema' => $schema, 'post_id' => $this->query->post->ID), $this->options));
                        echo "$tab\t</$html>\n";
                }
                echo "$tab</section>\n";
...
class byobagn_top_social_media_extender extends thesis_box {
        ...
        public function html($args = false) {
                ...
                echo $this->rotator(array_merge($args, array('depth' => $depth + 1, 'options' => $this->sharing_options)));
                echo "$tab</p>\n";
        }
...
class byobagn_comment_form extends thesis_box {
        ...
        public function html($args = array()) {
                ...
                if (get_option('comment_registration') && !!!$user_ID) #wp
                        ...
                        if (class_exists('GdbcWordPressPublicModule') && is_callable(array(GdbcWordPressPublicModule::getInstance(), 'renderTokenFieldIntoCommentsForm'))) {
